...
- [ ] Java 集合
    - [ ] java.util 包中存放集合类，有3种：set、list（包含queue）和 map
        - Collection 接口，是List、Set、Queue 的最基本的接口
        - Iterator 迭代器 实现迭代器遍历集合中的元素
        - Map 映射表的基础接口
...
- [ ] Java 并发
    ...
    - [ ] 继承 Thread 类
        - 无返回值
        - 本质上实现了 Runnable 接口的一个实例，代表一个线程的实例。
        - Thread 类的 start() 实例方法启动线程，并执行 run() 方法
    - [ ] 实现 Runnable 接口
        - 无返回值
        - 实例化 Thread，传入 MyThread 实例
        - Thread 类的 start() 实例方法启动线程，并执行 run() 方法
    - [ ] 实现 Callable 接口 
        - 有返回值
        - 结合线程池接口 ExecutorService
    - [ ] 线程池的方式 ExecutorService
        - newCachedThreadPool  
        - newFixedThreadPool 
...
- TCP / IP 
    - [ ] 分层
        ...
        - 传输层 
            ...
            - UDP 面向无连接的不可靠传输的协议
        - 网络层 IP 
        - 网络接口层 使用某种协议与网络相连 HTTP 协议    
    - [ ] TCP 三次握手
        - 第一次握手 主机 A 发送码为 syn = 1，随机产生 seq number = 1234567 的数据包到服务器，主机 B 由 SYN = 1 直达，A 要求建立连接
    ...
    - [ ] TCP 四次挥手
        - TCP 的半关闭造成的。因为 TCP 连接是全双工的（即数据可在两个方向上同时传递）所以进行关闭时每个方向都要单独进行关闭。
        - 当一方完成它的数据发送任务，就发送一个 FIN 来向另一方通告将要终止这个方向的连接
        - 第一次挥手 关闭客户端到服务器的连接 客户端 A 发送一个 FIN，用来关闭客户端要服务器的数据传送，然后等待服务器的确认。其中终止标志位 FIN = 1，序列号 seq = u
        - 第二次挥手 服务器端收到 FIN，回发一个 ACK，确认号 ack 为 seq + 1；
        - 第三次挥手 关闭服务器到客户端的连接，发送 FIN 给客户端
        - 第四次挥手 客户端收到 FIN，回发 ACK 报文，seq + 1

- HTTP 原理
    - [ ] 无状态协议，指的是浏览器和服务器之间不需要建立持久的连接，这意味着客户端向服务端发出请求后，然后服务器响应，连接就关闭了，在服务器端不会保留连接的有关信息，所有 HTTP 连接都被构造成一套请求和应答
    - [ ] 传输流程
        - 1. 地址解析 通过请求的URL分解出协议名、主机名、端口、路径等，还要通过 DNS 将域名解析为主机地址
        - 2. 封装 HTTP 请求数据包 将上述信息封装成一个 HTTP 请求数据包
        - 3. 封装成 TCP 包并建立连接 封装成 TCP 包，通过三次握手建立 TCP 连接
        - 4. 客户机发送请求命令 
        - 5. 服务器响应
        - 6. 服务关闭 TCP 连接 如果浏览器或服务器在头信息加入 Connection：keep-alive，TCP连接在发送后将仍然保持打开状态，于是，浏览器可以继续通过相同的连接发送请求，保持连接节省了为了每个请求建立起连接所需的时间，还节约了网络带宽
    - [ ] HTTP 状态码
        - 100 Continue
        - 101 切换协议
        - 200 成功
...
- 数据库
    ...
    - [ ] 索引原则
        - 1. 建立索引的字段值是唯一的，可以更快速的通过该索引来确定某条记录
        ...
        - 9. 尽可能扩展索引，不建立新索引
    - [ ] 数据库三范式（建表）
        - 1. 第一范式 确保每列的原子性：如果每列都是不可再分的最小数据单元
        - 2. 第二范式 每个表只描述一件事情，首先要满足第一范式
        - 3. 第三范式 不存在对非主键列的传递依赖  要满足第二范式 
    - [ ] 事务
        - 单个逻辑工作单元执行的一系列操作，这些操作作为一个整体向系统提交，要么执行，要么不执行
        - ACID 属性：
            - 原子性
            - 一致性
            - 隔离性
...
- [ ] 常用注解
    - [ ] @Controller 
        - 标注控制层组件
        - 分发处理器将会扫描使用了这个注解的类里的方法，并检测方法是否使用了@RequestMapping注解
    - [ ] @RestController 
        - @Controller 和 @responseBody 组合
    - [ ] @Component 
        - 通用组件，表示这个类被 Spring 管理
    - [ ] @Repository 
        - 注解 dao 层
    - [ ] @Service
        - 标注业务服务层组件
    - [ ] @responseBody 
        - 将@Controller 方法返回的对象，通过 HttpMessageConverter 转换为指定格式，写入到 Response 对象的 body 数据区，一般类型为 json 或  xml 等
    - [ ] @RequestMapping
        - 用于类和方法，用于类时表示所有请求的父路径
    - [ ] @Autowired 自动装配注入某个类
